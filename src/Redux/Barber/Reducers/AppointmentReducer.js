...
export const AppointmentListBarberReducer = (state = initialState, action) => {
    switch (action.type) {
        case GET_APPOINTMENT_LIST_BARBERID_APPOINTMENT_REQ:
            return { ...state, loading: true, error: null }; // Also good practice to clear a previous error on a new request
        case GET_APPOINTMENT_LIST_BARBERID_APPOINTMENT_SUCCESS:
            return { ...state, loading: false, response: action.payload.response, error: null };
        case GET_APPOINTMENT_LIST_BARBERID_APPOINTMENT_FAIL:
            return { ...state, loading: false, error: action.payload };

        // New case to clear the state
        case GET_APPOINTMENT_LIST_BARBERID_APPOINTMENT_CLEAR: // Use your actual constant here
            return initialState;
