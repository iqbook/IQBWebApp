...
const AppointmentList = () => {
    ...
    useEffect(() => {
        const fetchSalonMaxAppointmentDays = async () => {
            try {
                const { data } = await api.post('/api/salonSettings/getSalonSettingsForBarber', { salonId });
        ...
        <section className={style.appointmentSection}>
            <div className={style.calenderDayCalender}>
                <div>
                    ...
                    <div>
                        <button onClick={prevMonthFunc} disabled={isPrevDisabled}><LeftIcon size={"1rem"} /></button>
                        <button
