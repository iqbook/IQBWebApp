...
import api from "../../../Redux/api/Api";
import {
  CLEAR_ALL_APPOINTMENTSLIST,
  GET_APPOINTMENTSLIST_BY_BARBERNAME,
...
const AppointmentCalender = () => {
    ...
    const fetchAppointments = async () => {
      ...
      try {
        const { data } = await api.post(
          "/api/appointments/getAllAppointmentsBySalonId",
          { salonId },
          { signal }
        );
        setAppointmentData(data?.response || []);
  ...
  useEffect(() => {
    if (selectedDay) {
      dispatch(
        getAdminAppointmentListSalonIdAction({ salonId }, selectedDay.fullDate)
      );
    }
  ...
  const getAdminAppointmentListSalonId = useSelector(
    (state) => state.getAdminAppointmentListSalonId
  );
  const {
    loading: getAdminAppointmentListSalonIdLoading,
    response: getAdminAppointmentListSalonIdResponse,
