...
const AppointmentList = () => {
    ...
    try {
      ...
      const getAppointmentList = async () => {
        const { data } = await api.post(
          "/api/appointments/getAllAppointmentsBySalonIdAndDate",
          {
            salonId,
  ...
  return (
    <div className={`${style.section} ${darkmodeOn && style.dark}`}>
      ...
      <div
        className={`${style.appointment_content_wrapper} ${
        ...
        {loading ? (
          ...
          appointmentList?.response?.map((appoint) => (
            <React.Fragment key={appoint.barberId}>
              <main
                className={`${style.appointment_container} ${
                  darkmodeOn ? style.dark : ""
                }`}
              >
                <div
                  className={`${style.appointment_barber_container} ${
                    darkmodeOn ? style.dark : ""
                  }`}
                >
                  <div>
                ...
                <main className={style.appointment_content_list_container}>
                  {appoint?.appointments.map((cus) => (
                    <div
                      className={`${style.appointment_body_customer_item} ${
                        darkmodeOn ? style.dark : ""
                      }`}
                      key={cus._id}
                    >
                      ...
                      <div>
                        ...
                        <p>
                          ...
                          {formatMinutesToHrMin(
                            cus.services.reduce(
                              (total, service) =>
                                total + service.barberServiceEWT,
                              0
                            )
                          )}
                        </p>
          ...
          <div className={style.list_container_error}>
      ...
      {loading ? (
        <div
          className={`${style.appointment_mobile_content_wrapper_loading} ${
            darkmodeOn && style.dark
          }`}
        >
          <Skeleton
        ...
        <div
          className={`${style.appointment_mobile_content_wrapper} ${
            darkmodeOn && style.dark
          }`}
        >
          {appointmentList?.response?.map((appoint, index) => {
            return (
              <Accordion
                key={appoint.barberId}
                expanded={expanded === `panel${appoint.barberId}`}
                onChange={handleChange(`panel${appoint.barberId}`)}
              >
                <AccordionSummary
                ...
                >
                  ...
                  >
                    <img
                      ...
                      src={appoint.barberProfile?.[0]?.url}
                      alt="profile"
                    />
                  </div>
                  <Typography
                ...
                >
                  {appoint.appointments.map((cus, index) => {
                    return (
                      <div
                        className={`${
                          style.appointment_body_customer_mobile_item
                        } ${darkmodeOn ? style.dark : ""}`}
                        key={index}
                        style={{
                          borderBottom:
                            index === appoint.appointments.length - 1 && "none",
                        }}
                      >
                        <div>
                          <div>
        ...
        <div
          className={`${style.appointment_mobile_content_wrapper_error} ${
            darkmodeOn && style.dark
          }`}
        >
          <p>No appointment available</p>
